...
Required action:
   If you do not use the Toolchain Manager, install the Zephyr SDK by following the steps in :ref:`gs_installing_toolchain`.
...
* The zcbor state ``zs`` consists of two states, which is a minimal practical state count.
...
* The zcbor state ``zs`` consists of two states, which is enough when zcbor is in non-canonical configuration (see :kconfig:option:`CONFIG_ZCBOR_CANONICAL`).
  In non-canonical configuration, the number of states would need to be increased by a maximum level of expected lists and/or arrays enveloping each other.
* The ``buffer`` where encoded data will be placed, with size that limits it.
* The ``0`` is the final parameter that represents the number of items encoded into the buffer.
...
Following is the zcbor conversion of the given code example:
...
* The zcbor decoding context, ``zsd`` in the above example
* The mapping specification and size of the mapping
* A pointer to a variable for number of successfully decoded map elements

.. note::
   Currently, there is no method provided to determine the fields that are not found in the string.
